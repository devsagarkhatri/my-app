...
There is also a particular built-in environment variable called `NODE_ENV`. You can read it from `process.env.NODE_ENV`. When you run `npm start`, it is always equal to `'development'` when you run `npm test` it is still comparable to `'test'`, and when you run `npm run build` to make a production bundle, it is always similar to `'production'`. **You cannot override `NODE_ENV` manually.** This prevents developers from accidentally deploying a slow development build to production.
